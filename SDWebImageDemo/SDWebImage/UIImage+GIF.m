...
+ (int)skipFramesStep:(UIImage *)image imageCount:(int)count {
    size_t imageSize = CGImageGetBytesPerRow(image.CGImage) * CGImageGetHeight(image.CGImage);
    int maxFrames = 20.0*1024*1024/imageSize;
    int step = ceilf(count*1.0/maxFrames);
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (i == 0 || (i % step == 0)) {
                ...
                if (i == 0) {
                    step = [self skipFramesStep:uiimage imageCount:count];
                }
